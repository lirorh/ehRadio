...
/*****************************************************************************************************************************************************
    ...
    Version 3.4.6w
  Updated on: Jun 18, 2026, 21.06.2026 (Maleksm)
...
//    m_nominal_bitrate = (m_decoder->getBitRate());
...
void Audio::calculateVUlevel(int32_t* sample) { // Envelope-Follower
    ...
    if (level > 1e-6f) {
        ...
        m_fft_items.gain += (desired < m_fft_items.gain ? ATTACK : RELEASE_FFT) * (desired - m_fft_items.gain);
    }
    if (level < 1e-4f) mono = 0.0f;
    ...
    // Calculation VU
//    int8_t l = abs(m_vu_items.delay_l[pos] >> 23);
//    int8_t r = abs(m_vu_items.delay_r[pos] >> 23);

    // Optimization (abs) for int32_t abs(x >> 23) is better done through a condition or mask, 
    int32_t raw_l = m_vu_items.delay_l[pos] >> 23;
    int32_t raw_r = m_vu_items.delay_r[pos] >> 23;
    
    uint8_t l = (uint8_t)(raw_l > 0 ? raw_l : -raw_l);
    uint8_t r = (uint8_t)(raw_r > 0 ? raw_r : -raw_r);

    // Attack/Release
    constexpr float RELEASE = 1.0f; // the bigger, the more sluggish
    if (l > m_vu_items.left) m_vu_items.left = l;
    else if (m_vu_items.left > RELEASE) m_vu_items.left -= RELEASE;
    if (r > m_vu_items.right) m_vu_items.right = r;
    else if (m_vu_items.right > RELEASE) m_vu_items.right -= RELEASE;
...
//    return ((uint8_t)m_vu_items.right_peak << 8) + (uint8_t)m_vu_items.left_peak; // returns  rrrrrrrrllllllll
    return ((uint8_t)m_vu_items.left_peak << 8) + (uint8_t)m_vu_items.right_peak; // returns  rrrrrrrrllllllll
}
//*************************************************************************************************************
...
//    constexpr float BALANCE_DB = -16.0f;
    ...
    // 1. Simplified loudness calculation (quadratic curve)
    // It feels much more linear than the logarithmic curve
    auto volumeToLinear = [&](uint8_t volume, uint8_t steps) {
        if (volume == 0) return 0.0f; 						// real silence
        float t = (float)volume / (float)steps;
        return t * t; // Quadratic dependence (t^2)
    };

    float vol = volumeToLinear(m_audio_items.cur_volume, m_audio_items.volume_steps);

    // 2. Balance calculation (without powf)
    // It's best to make the balance smooth. For example, a damping factor of 0.15 (similar to BALANCE_DB)
    float l_mul = 1.0f;
    float r_mul = 1.0f;

    // We assume that the balance is in the range of -16 ... 16
    float balance_factor = fabsf((float)m_audio_items.balance / 16.0f);
    float reduction = 1.0f - (balance_factor * 0.85f); // We attenuate the channel to 15% volume maximum

    if (m_audio_items.balance > 0) { // More to the right -> quieter to the left
        l_mul = reduction;
    } else if (m_audio_items.balance < 0) { // Left -> right quieter
        r_mul = reduction;
    }

    // 3. Final limiters
    m_audio_items.limiter[LEFTCHANNEL] = vol * l_mul;
    m_audio_items.limiter[RIGHTCHANNEL] = vol * r_mul;
