...
void Audio::setBalance(int8_t bal){ 
        if (bal > 16) { m_balance = 16;
        } else if (bal < -16) { m_balance = -16;
        } else {
            m_balance = bal;
        }
	setVolume(curvol);
...
int Audio::findNextSync(uint8_t* data, size_t len) {
    ...
    if(m_codec == CODEC_WAV) { m_f_playing = true;
        nextSync = 0;
    }
    ...
    if(m_codec == CODEC_M4A) { m_f_playing = true;
        nextSync = 0;
    }
    if(m_codec == CODEC_VORBIS) { nextSync = VORBISFindSyncWord(data, len);
        if(nextSync == -1) return len; 						// OggS not found, search next block
    }
    if(m_codec == CODEC_FLAC) { nextSync = FLACFindSyncWord(data, len);
        if(nextSync == -1) return len; 						// OggS not found, search next block
    }
    if(m_codec == CODEC_OPUS) { nextSync = OPUSFindSyncWord(data, len);
        if(nextSync == -1) return len; 						// OggS not found, search next block
    }
...
//###################################################################
/*
int32_t Audio::VORBISFindSyncWord(unsigned char *buf, int32_t nBytes){
//int32_t Audio::VORBISFindSyncWord(uint8_t *buf, int32_t nBytes){
...
//###################################################################
/*
int32_t Audio::FLACFindSyncWord(unsigned char *buf, int32_t nBytes) {
//int32_t Audio::FLACFindSyncWord(uint8_t *buf, int32_t nBytes) {

    int32_t idx = specialIndexOf(buf, "OggS", nBytes);
    if(idx == 0) return 0; 				// flag has ogg wrapper
    if(idx > 0) return idx;
    else{
//         find byte-aligned sync code - need 14 matching bits
        for (idx = 0; idx < nBytes - 1; idx++) {
            if ((buf[idx + 0] & 0xFF) == 0xFF  && (buf[idx + 1] & 0xFC) == 0xF8) { 	// <14> Sync code '11111111111110xx'
//                if(idx) FLACDecoderReset();
                return idx;
            }
        }
    }
...
//###################################################################
/*
int32_t Audio::OPUSFindSyncWord(unsigned char *buf, int32_t nBytes){
//int32_t Audio::OPUSFindSyncWord(uint8_t *buf, int32_t nBytes){
...
//    }
    ...
    switch(m_codec) {
        case CODEC_FLAC:
//                            if(parseFlacComment(data, len) == 1) {
//                            parseFlacComment(data, len);
                            st = FLACgetStreamTitle();
                            if(st) { AUDIO_INFO(st); if(audio_showstreamtitle) audio_showstreamtitle(st); }
                            break;
//                            }
        case CODEC_OPUS:
//                            if(parseOpusComment(data, len) == 1) {
...
//                            }
        case CODEC_VORBIS:
//                   AUDIO_INFO("Starting parseVorbisComment()");
//                            if(parseVorbisComment(data, len) == 1) {
//                   AUDIO_INFO("parseVorbisComment() is done");
//                            parseVorbisComment(data, len);
                            st = VORBISgetStreamTitle();
                   AUDIO_INFO("Comment string: %s, st");
                            if(st) { AUDIO_INFO(st); if(audio_showstreamtitle) audio_showstreamtitle(st); }
                            break;
//                            }
    }
...
//    if(getBitRate()) { AUDIO_INFO("BitRate: %lu", (long unsigned int)m_bitRate); }
//    else { AUDIO_INFO("BitRate: N/A"); }
//    if(h_bitRate) {AUDIO_INFO("BitRate: %lu", (long unsigned int)h_bitRate); }
    if(!h_bitRate) { AUDIO_INFO("BitRate: %lu", (long unsigned int)getBitRate()); }
    if(!m_bitRate) { AUDIO_INFO("BitRate: N/A"); }
...
void Audio::processLocalFile() {
    ...
    const uint32_t  timeout = 4500; 					// ms
...
//    if(!m_contentlength && !m_f_tts) {log_e("webfile without contentlength!"); stopSong(); return;} 		// guard
    ...
    if(m_codec == CODEC_OGG) { AUDIO_INFO("format is ogg"); 			// log_i("determine correct codec here");
        uint8_t codec = determineOggCodec(InBuff.getReadPtr(), maxFrameSize);
...
void Audio::playAudioData() {
    ...
    if(m_f_firstPlayCall) {
	    ...
	    if(m_validSamples) return;		    // play samples first
    if(m_f_eof) return;
...
bool Audio::readPlayListData() {
    ...
    while(true){  // outer while
        ...
        if(startsWith(pl, "<!DOCTYPE")) {AUDIO_ERROR("url is a webpage!"); goto exit;}
        if(startsWith(pl, "<html"))     {AUDIO_ERROR("url is a webpage!"); goto exit;}
...
//        int res = parseFlacComment(InBuff.getReadPtr(), bytes);
        if(res >= 0) bytesReaded = res;
        else { 				// error, skip header
            stopSong();
            m_controlCounter = 100;
...
//        int res = read_OPUS_Header(InBuff.getReadPtr(), bytes);
        int res = read_FLAC_Header(InBuff.getReadPtr(), bytes);
//        int res = parseOpusComment(InBuff.getReadPtr(), bytes);
        if(res >= 0) bytesReaded = res;
        else { 				// error, skip header
            stopSong();
            m_controlCounter = 100;
...
//        int res = read_VORBIS_Header(InBuff.getReadPtr(), bytes);
        int res = read_FLAC_Header(InBuff.getReadPtr(), bytes);
//        int res = parseVorbisComment(InBuff.getReadPtr(), bytes);
        if(res >= 0) bytesReaded = res;
        else { 				// error, skip header
            stopSong();
            m_controlCounter = 100;
...
//        setBitrate(nic * sr * bps);
        ...
        //    AUDIO_INFO("BitRate: %lu", (long unsigned int)m_bitRate);
        headerSize += 16;
        return 16; 							// ok
    }
...
//        FLACSetRawBlockParams(m_flacNumChannels, m_flacSampleRate, m_flacBitsPerSample, m_flacTotalSamplesInStream, m_audioDataSize);
    ...
    if(m_controlCounter == FLAC_VORBIS) { 				/* VORBIS COMMENT */ // field names
        ...
        for(int i = 0; i < commentListLength; i++) {
            ...
            if(startsWith(m_chbuf, "album") || startsWith(m_chbuf, "ALBUM")){
                album = strdup(m_chbuf + 6);
                if(audio_id3album) audio_id3album(album); }

            if(startsWith(m_chbuf, "artist") || startsWith(m_chbuf, "ARTIST")){
                artist = strdup(m_chbuf + 7);
                if(audio_id3artist) audio_id3artist(artist); }

            if(startsWith(m_chbuf, "title") || startsWith(m_chbuf, "TITLE")){
                title = strdup(m_chbuf + 6);
                strcpy(album, " - ");
                strcpy(album, title);
                if(audio_id3album) audio_id3album(album); }
...
int Audio::read_ID3_Header(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == 0){      				/* read ID3 tag and ID3 header size */
        ...
        if(!m_f_m3u8data) { AUDIO_INFO("ID3 framesSize: %i", id3Size); AUDIO_INFO("ID3 version: 2.%i", ID3version); }
...
//            }
        ...
        if(encodingByte == 0){  // latin
            latinToUTF8(m_ibuff, m_ibuffSize, false); 		// convert unicode to utf-8 U+0020...U+07FF
...
//            setBitrate(bps * channel * srate);
            ...
            sprintf(m_chbuf, "%lu", (long unsigned int)m_bitRate);
    ...
    if(m_controlCounter == M4A_MDAT) { 						 // mdat
        m_audioDataSize = bigEndian(data, 4); 					// length of this atom - strlen(M4A_MDAT)
...
const char* Audio::parsePlaylist_M3U(){
    ...
    for(int i= 0; i < lines; i++){
        ...
        if(pos >= 0) {
            AUDIO_INFO("Entry in M3U-playlist found: %s", (m_playlistContent[i] + pos + 9));
        ...
        if(pos >= 0) { 								// Does URL contain "http://"?
            ...
            AUDIO_INFO("Entry in M3U-playlist found: %s", (m_playlistContent[i] + pos));
...
//            if(strcmp(m_playlistContent[0] , "[playlist]") != 0 )) 	// first entry in valid pls
                ...
                AUDIO_INFO("PLS-list is not valid, switch to HTTP_RESPONSE_HEADER");
                goto exit;
            }
         continue;
        }
        if(startsWith(m_playlistContent[i], "File")) {
        ...
        if(startsWith(m_playlistContent[i], "Title")) { 		// Title1=Antenne Tirol
        ...
        if(startsWith(m_playlistContent[i], "Length")){ continue; }
        ...
        if(f_EXTINF_found){
            if(f_mediaSeq_found){
                ...
                if(xMedSeq == 0 || xMedSeq == UINT64_MAX) {
                     log_e("xMediaSequence not found"); connecttohost(m_lastHost);}
                if(mediaSeq < xMedSeq){
    ...
    while(true) { 							// outer while
        ...
        if(startsWith(rhl, "HTTP/")) { 			// HTTP status error code
            ...
            if(parseContentType(rhl + 13)) ct_seen = true;
                ...
                AUDIO_ERROR("unknown contentType %s", rhl + 13);
                goto exit;
            }
        }
...
//            sprintf(m_chbuf, "%lu", (long unsigned int)getBitRate());
            sprintf(m_chbuf, "%lu", (long unsigned int)h_bitRate);
            if(audio_bitrate) audio_bitrate(m_chbuf);
        }
...
bool Audio::parseContentType(char* ct) {
    ...
    enum : int {CT_NONE, CT_MP3, CT_AAC, CT_M4A, CT_WAV, CT_FLAC, CT_PLS, CT_M3U, CT_ASX, CT_M3U8, CT_TXT, CT_AACP, CT_OGG, CT_VORBIS, CT_OPUS};
...
//    else if(!strcmp(ct, "video/mp2t")){      ct_val = CT_AAC; m_f_ts = true;} // assume AAC transport stream
    ...
    else if(!strcmp(ct, "audio/x-mpegurl")) ct_val = CT_M3U; 	//if(m_expectedPlsFmt == FORMAT_M3U8) ct_val = CT_M3U8;}
    ...
    switch(ct_val){
        ...
        case CT_TXT: // overwrite text/plain
            if(m_expectedCodec == CODEC_AAC) {
                m_codec = CODEC_AAC; audio_info("format is aac"); if(m_f_Log) log_i("set ct from M3U8 to AAC");}
            if(m_expectedCodec == CODEC_MP3) {
                m_codec = CODEC_MP3; audio_info("format is mp3"); if(m_f_Log) log_i("set ct from M3U8 to MP3");}
...
//    m_f_running = false;       		// already done in stopSong
    ...
    h_bitRate = 0; 				// Bitrate from header still unknown
...
//    else                            {m_f_ssl = false; hostwoext_begin = 7; port = 80;}
    else  {m_f_ssl = false; port = 80;
        if (startsWith(h_host, "www")) hostwoext_begin = 4;
        else hostwoext_begin = 7;
    }

    // In the URL there may be an extension, like noisefm.ru:8000/play.m3u&t=.m3u
    pos_slash     = indexOf(h_host, "/", 10); 				// position of "/" in hostname
    pos_colon     = indexOf(h_host, ":", 10); 				// position of ":" in hostname
    ...
    if((pos_colon > 0) && ((pos_ampersand == -1) || (pos_ampersand > pos_colon))) {
        ...
        h_host[pos_colon] = '\0'; 							// Host without portnumber
    }
    setDefaults();
    ...
    if(res) {
        ...
        if(endsWith(h_host, ".vorbis"))   m_expectedCodec = CODEC_VORBIS;
        if(endsWith(h_host, "/vorbis"))   m_expectedCodec = CODEC_VORBIS;
        ...
        AUDIO_ERROR("Request %s failed!", c_host);
        m_f_running = false;
        if(audio_showstation) audio_showstation("");
...
bool Audio::httpPrint(const char* host) {
    ...
    else if(endsWith(extension, ".aacp"))       m_expectedCodec  = CODEC_AAC;
    ...
    else if(endsWith(extension, ".vorbis"))   m_expectedCodec = CODEC_VORBIS;
    else if(endsWith(extension, "/vorbis"))   m_expectedCodec = CODEC_VORBIS;
    else if(endsWith(extension, ".opus"))   m_expectedCodec = CODEC_OPUS;
    else if(endsWith(extension, "/opus"))   m_expectedCodec = CODEC_OPUS;
    else if(endsWith(extension, ".ogg" ))   m_expectedCodec = CODEC_OGG;
    else if(endsWith(extension, "/ogg" ))   m_expectedCodec = CODEC_OGG;
...
log_e("%s", rqh);
    ...
    if(endsWith(extension, ".vorbis"))      m_expectedCodec  = CODEC_VORBIS;
...
//###################################################################
bool Audio::connecttoSD(String sdfile, int32_t resumeFilePos) {
    return connecttoFS(SD, sdfile.c_str(), resumeFilePos); }

bool Audio::connecttoSD(const char* sdfile, int32_t resumeFilePos) {
...
bool Audio::connecttoFS(fs::FS &fs, const char* path, int32_t fileStartPos) {
    ...
    if(endsWith(path, ".mp3")) { codec = CODEC_MP3; res = true; audio_info("format is mp3"); }
//    if(endsWith(path, ".m4a")) { codec = CODEC_M4A; res = true; audio_info("format is aac"); }
//    if(endsWith(path, ".aac")) { codec = CODEC_AAC; res = true; audio_info("format is aac"); }
//    if(endsWith(path, ".wav")) { codec = CODEC_WAV; res = true; audio_info("format is wav"); }
    if(endsWith(path, ".flac")) { codec = CODEC_FLAC; res = true; audio_info("format is flac"); }
    if(endsWith(path, ".opus")) { codec = CODEC_OPUS; res = true; audio_info("format is opus"); }
    if(endsWith(path, ".ogg")) { codec = CODEC_OGG; res = true; audio_info("format is ogg"); }
//    if(endsWith(path, ".oga")) { codec = CODEC_OGG; res = true; audio_info("format is ogg"); }

    if(codec == CODEC_NONE) {AUDIO_ERROR("The %s format is not supported", path + dotPos); res = false; goto exit;}   // guard
...
void Audio::showID3Tag(const char* tag, const char* value){
    ...
    if(!strcmp(tag, "TAL")) { sprintf(m_chbuf, "Album/Movie/Show title: %s", value); if(audio_id3album) audio_id3album(value); }
    ...
    if(!strcmp(tag, "TOA")) { sprintf(m_chbuf, "Original artist(s)/performer(s): %s", value); if(audio_id3artist) audio_id3artist(value); }
...
uint32_t Audio::getFileSize(){ 				// returns the size of webfile or local file
    if (!audiofile) {  if (m_contentlength > 0) { return m_contentlength;}  return 0; }
...
//###################################################################
bool Audio::setTimeOffset(int16_t sec) { 			// fast forward or rewind the current position in seconds
...
//    m_avr_bitrate = (wram_read(0x1e05) & 0xFF) * 1000; 		// Read average bitRate from WRAM register
  ...
  if((prev_bitrate /1000) == (m_avr_bitrate / 1000)) m_localBitrateSend = true;
  if(audio_bitrate && !m_localBitrateSend) {
    sprintf(m_chbuf, "%lu", (long unsigned int)m_avr_bitrate);
    audio_bitrate(m_chbuf);
    m_localBitrateSend = true;
  }
  m_audioFileDuration = 8 * ((float)m_audioDataSize / (m_avr_bitrate));
  m_audioCurrentTime = round(8 * ((float)getFilePos() / (m_avr_bitrate)));
...
void Audio::seek_m4a_stsz() {
    ...
    /* atom hierarchy (example)___________________________________________________________
...
//        uint8_t  mpegVers = (verIdx == 0 ? MPEG25 : ((verIdx & 0x01) ? MPEG1 : MPEG2));
        uint8_t  mpegVers = (verIdx == 0 ? 2 : ((verIdx & 0x01) ? 0 : 1));
