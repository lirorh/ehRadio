...
Audio::Audio(uint8_t _cs_pin, uint8_t _dcs_pin, uint8_t _dreq_pin, SPIClass *spi)
{
    ...
    // [Maleksm graft] task coordination mutexes
    ...
    // [Maleksm graft] start FreeRTOS audio task for anti-skip feeding
    startAudioTask();
}
Audio::~Audio(){
    // destructor
    // [Maleksm graft] stop task before freeing resources
    stopAudioTask();
...
void Audio::setVolume(uint8_t vol){
    ...
    // Input value is 0..VOLUME_SCALE which determines the loudest volume
    ...
    if(m_f_running) {
        ...
        for(i=0; i < 200; i++) {
            ...
            if((modereg & _BV(SM_CANCEL)) == 0) {
                ...
                if(audio_info) audio_info(m_chbuf);
                // [Maleksm graft] always clear SM_CANCEL to prevent stuck bit
                write_register(SCI_MODE, _BV (SM_SDINEW) | _BV(SM_LINE1));
                return;
            }
            delay(10);
        ...
        // [Maleksm graft] force-clear SM_CANCEL even after failed drain
        write_register(SCI_MODE, _BV (SM_SDINEW) | _BV(SM_LINE1));
...
// [Maleksm graft] setAudioTaskCore now in task infrastructure section at end of file
//---------------------------------------------------------------------------------------------------------------------
...
void Audio::processLocalFile() {
    ...
    if(!f_stream && m_controlCounter == 100) {
        f_stream = true;
        m_f_stream_ready = true; // [Maleksm graft] signal FreeRTOS task that data is available
...
void Audio::processWebStream() {
    ...
    if(availableBytes) {
        ...
        if(InBuff.bufferFilled() > maxFrameSize && !f_stream) {  // waiting for buffer filled
            ...
            m_f_stream_ready = true; // [Maleksm graft] signal FreeRTOS task
    ...
    if(f_stream){
        ...
        cnt++;
// [Maleksm graft] if(cnt == 3){playAudioData(); cnt = 0;} // task handles this via performAudioTask()
    }
}
//---------------------------------------------------------------------------------------------------------------------
...
// [Maleksm graft] if(InBuff.freeSpace() < maxFrameSize && f_stream){playAudioData(); return;} // task handles this
    ...
    if(f_stream){
        static uint8_t cnt = 0;
        cnt++;
// [Maleksm graft] if(cnt == 1){playAudioData(); cnt = 0;} // aac only — task handles this
    }
    return;
}
//---------------------------------------------------------------------------------------------------------------------
void Audio::processWebStreamHLS() {
    ...
    if(f_stream){
        static uint8_t cnt = 0;
        cnt++;
// [Maleksm graft] if(cnt == 1){playAudioData(); } // aac only — task handles this
    }
    return;
}
//---------------------------------------------------------------------------------------------------------------------
void Audio::processWebFile(){
    ...
    if(f_stream){
        ...
        cnt++;
// [Maleksm graft] if(cnt == compression){playAudioData(); cnt = 0;} // task handles this
    }
    return;
}
//---------------------------------------------------------------------------------------------------------------------
...
// [Maleksm graft] task-coordinated playAudioData — called ONLY from the FreeRTOS audio task
void Audio::playAudioData() {

    if(!m_f_stream_ready) return; // [Maleksm graft] adapted: PR226 uses m_f_stream_ready instead of m_f_stream
    ...
    if(m_f_firstPlayCall) {
        ...
        else if(getDatamode() == AUDIO_LOCALFILE) f_isFile = true;
...
// [Maleksm graft] playAudioData(); // avoid audio gap — task handles this via performAudioTask()
    return NULL;
}
//---------------------------------------------------------------------------------------------------------------------
...
void Audio::setDefaults(){
    ...
    // [Maleksm graft] coordinate with audio task
    m_f_lockInBuffer = false;
    m_f_stream_ready = false;  // reset stream-ready flag for new connection
    m_f_eof = false;
...
// [Maleksm graft] VU meter — improved: full 0-95dB range, peak-hold with decay
//------------------------------------------------------------------------------
void Audio::setVUmeter() {
  // enable VSdsp VU Meter — writes SS_VU_ENABLE bit of SCI_STATUS
...
int Audio::read_ID3_Header(uint8_t *data, size_t len) {
    ...
    if(m_controlCounter == 0){      /* read ID3 tag and ID3 header size */
        if(getDatamode() == AUDIO_LOCALFILE){
            ...
            ID3version = 0;
            sprintf(m_chbuf, "Content-Length: %lu", m_contentlength);
            if(audio_info) audio_info(m_chbuf);
        }
        m_controlCounter ++;
        SYLT_seen = false;
...
//###################################################################
// [Maleksm graft] FreeRTOS audio task — feeds VS1053 independently of Arduino loop
// This prevents audio skipping when display updates or network operations
// block the Arduino loop() for extended periods.
//****************************************************************************************
void Audio::setAudioTaskCore(uint8_t coreID){
    // Recommendation: if Arduino loop runs on Core 1, audio task should be Core 0 or vice versa
    if(coreID > 1) return;
    stopAudioTask();
...
void Audio::performAudioTask() {
    if(!m_f_running) return;
    if(!m_f_stream_ready) return; // [Maleksm graft] adapted for PR226
    if(m_codec == CODEC_NONE) return; // wait for codec is set
    if(m_codec == CODEC_OGG)  return; // wait for FLAC, VORBIS or OPUS (handled by inline parser)
    xSemaphoreTake(mutex_audioTask, 0.3 * configTICK_RATE_HZ);
