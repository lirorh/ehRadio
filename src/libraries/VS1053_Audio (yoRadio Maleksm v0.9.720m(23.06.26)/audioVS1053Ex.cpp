...
/*****************************************************************************************
   ...
   Version 3.4.6w   Updated on: 21.06.2026 (Maleksm)
...
#include "../audioI2S/wav_decoder/wav_decoder.h"
...
//###################################################################
// üìåüìåüìå  A U D I O B U F F E R  üìåüìåüìå
//###################################################################
...
Audio::Audio(uint8_t _cs_pin, uint8_t _dcs_pin, uint8_t _dreq_pin, SPIClass *spi) :
    ...
    AUDIO_LOG_ERROR("Arduino Version too old!");
#endif
...
/*    if (m_decoder && m_decoder->isValid()) {
    ...
    if (m_decoder) {
        AUDIO_LOG_INFO("Decoder has been destroyed");
        m_decoder = false;
    }
}
//###################################################################
...
//    write_register(SCI_AUDATA, 44100 + 1);                  // 44.1kHz + stereo
    write_register(SCI_AUDATA, 48000 + 1);                  // 48kHz + stereo
...
//    VS1053_SPI_DATA  = SPISettings(4710400, MSBFIRST, SPI_MODE0);
//    VS1053_SPI_DATA  = SPISettings(3300000, MSBFIRST, SPI_MODE0);

// --- —“¿ÀŒ ---
// «‡‰‡ÂÏ ÏÌÓÊËÚÂÎ¸ 4.5x + ‰Ó·‡‚ÍÛ Ì‡ ‡Á„ÓÌ (SC_MULT = 4.5x, SC_ADD = 2.0x)
// «Ì‡˜ÂÌËÂ 0xC000 ó ˝ÚÓ ÒÚ‡Ì‰‡Ú ‰Â-Ù‡ÍÚÓ ‰Îˇ FLAC ‚ VS1053
//write_register(SCI_CLOCKF, 0xC000);
write_register(SCI_CLOCKF, 0x6000); // ÒÚ‡˚È ‰Ó·˚È 6 << 12
//write_register(SCI_CLOCKF, 0x8800); // 4.5x ó ÒÚ‡Ì‰‡Ú ‰Îˇ ÔÎ‡„ËÌÓ‚
// “ÂÔÂ¸, ÍÓ„‰‡ ˜ËÔ ‡·ÓÚ‡ÂÚ Ì‡ ˜‡ÒÚÓÚÂ ~55 Ã√ˆ, Ï˚ ÏÓÊÂÏ ·ÂÁÓÔ‡ÒÌÓ 
// ‡ÁÓ„Ì‡Ú¸ SPI ¯ËÌÛ ‰‡ÌÌ˚ı ‰Ó 6.7 Ã√ˆ (ËÎË ‰‡ÊÂ 8 Ã√ˆ), ˜ÚÓ·˚ ‰‡ÌÌ˚Â ÔÓÎÂÚ‡ÎË ÔÛÎÂÈ.
VS1053_SPI_DATA  = SPISettings(6700000, MSBFIRST, SPI_MODE0);
...
void Audio::loadUserCode(void) {
  ...
  Serial.print("VS1053: UserCode is loaded\t");
}
//###################################################################
...
bool Audio::httpPrint(const char* host) {
    ...
    else if (extension.ends_with_icase(".aacp"))
        m_expectedCodec = CODEC_AAC;
	...
	m_expectedCodec = CODEC_FLAC;
    else if (extension.ends_with_icase(".vorbis"))
	m_expectedCodec = CODEC_VORBIS;
    else if (extension.ends_with_icase("/vorbis"))
	m_expectedCodec = CODEC_VORBIS;
    else if (extension.ends_with_icase(".opus"))
	m_expectedCodec = CODEC_OPUS;
    else if (extension.ends_with_icase("/opus"))
	m_expectedCodec = CODEC_OPUS;
    else if (extension.ends_with_icase(".ogg" ))
...
//    if(m_fileStartPos == 0) m_fileStartPos = -1;
    ...
    if (c_path.ends_with_icase(".opus")) m_codec = CODEC_OPUS;
    if (c_path.ends_with_icase(".vor")) m_codec = CODEC_VORBIS;
...
//            // info(*this, evt_info, "maxBitrate {}, nominalBitrate {}", m_m4aHdr.maxBitrate, m_m4aHdr.nomBitrate);
        ...
        if (dec_specific_offset >= 0) {                                                           // decoderSpecificInfo found
            uint8_t dec_specific_length = ((uint8_t*)esds_buffer.get())[dec_specific_offset + 4]; // L–ì¬§nge nach Tag + 3 Extended Length Bytes
...
//            sdi_send_fillers(2052);
            sdi_send_fillers(vs1053_chunk_size * 54);
            info(*this, evt_info, "Song stopped correctly after {} msec", i * 10);
            goto exit;
        }
        delay(10);
    }
    info(*this, evt_info, "Song stopped incorrectly!");
...
//    if(strlen(str)) AUDIO_LOG_INFO(str);
    info(*this, evt_info, "PrintDetails: {}", str);
    ...
    info(*this, evt_info, "REG            dec      bin               hex");
    info(*this, evt_info, "-----------  -------  ----------------  -------");

    for(i=0; i <= SCI_AICTRL3; i++){
        info(*this, evt_info, "{}   {}   {}   {}", regName[i], decbuf[i], binbuf[i], hexbuf[i]);
    }
}
//###################################################################
...
//###################################################################
/*
// * VLSI datasheet: "SM_STREAM activates VS1053b–≤–Ç‚Ñ¢s stream mode. In this mode, data should be sent with as
...
void Audio::processLocalFile() {
    ...
    if (!m_decoder && InBuff.bufferFilled() > 127) {
        if (!initializeDecoder()) {info(*this, evt_info, "Decoder can't be initialised"); return;}
    ...
    if (m_fileStartTime > 0 && m_nominal_bitrate) {
        if (getBitRate() > 0) {
            ...
            setAudioFilePosition(m_fileStartTime); }
        else
...
chunkFinished:
    ...
    // buffer fill routine  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    {
        if (InBuff.bufferFilled() > 120000 && !m_f_stream) { // waiting for buffer filled
            m_f_stream = true;                                                     // ready to play the audio data
...
//                if (m_pad.bytesToDecode >= 32) {
//                   // ¡ÂÂÏ ÍÛÒÓ˜ÂÍ ‰Ó 512 ·‡ÈÚ
//                   size_t bytesToSend = min((int32_t)m_pad.bytesToDecode, (int32_t)512);
//                   m_pad.bytesDecoded = sendBytes(InBuff.getReadPtr(), bytesToSend);
//                }
            }
        }
...
//if (millis() - lastLogTime > 1000) { // ‡Á ‚ ÒÂÍÛÌ‰Û
//    lastLogTime = millis();
//    info(*this, evt_info, "InBuff free: {}, InBuff available data: {}, DREQ: {}", 
//                   InBuff.writeSpace(), InBuff.readSpace(), digitalRead(dreq_pin));
//}
...
bool Audio::parseHttpResponseHeader() { // this is the response to a GET / request
    ...
    while (true) { // read the header line for line
        ...
        if (rhl.starts_with_icase("icy-")) { // -- ICY BLOCK ‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî
        ...
        } // -- END ICY BLOCK ‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî‚Äî
...
lastToDo:
    ...
    if (m_codec != CODEC_NONE) {
        ...
        AUDIO_LOG_ERROR("unknown content found at: {}", m_currentHost.c_get());
        goto exit;
    }
...
bool Audio::initializeDecoder() {
    ...
    if (type) {
        info(*this, evt_info, "Codec is {}", type);
        showCodecParams();
//        m_decoder = createDecoder(type);
        m_decoder = true;
//        if (m_codec == CODEC_FLAC) { info(*this, evt_info,"InitializeDecoder FLAC. Write to SCI_AIADDR-register -80-"); write_register(SCI_AIADDR, 80); }
...
//    info(*this, evt_info, "{}Decoder has been initialized", m_decoder->whoIsIt());
    info(*this, evt_info, "<{}>-Decoder has been initialized", type);
    return true;
}
//##################################################################
...
void Audio::showCodecParams() {
    ...
    if(getBitRate()) info(*this, evt_bitrate, "{}", getBitRate());
...
/*     const char*           st = NULL;
   ...
   switch (m_codec) {
        case CODEC_FLAC:
            ...
            FlacDecoder tempDecoder(*this);
            st = tempDecoder.getStreamTitle();
            if (st) { info(*this, evt_streamtitle, "{}", st); }
            break;
        case CODEC_VORBIS:
            //st = VORBISgetStreamTitle();
            VorbisDecoder tempDecoder(*this);
            st = tempDecoder.getStreamTitle();
            if (st) { info(*this, evt_streamtitle, st); }
            break;
//        case CODEC_OPUS:
//            //st = OPUSgetStreamTitle();
//            OpusDecoder tempDecoder(*this);
//            st = tempDecoder->getStreamTitle();
//            if (st) { info(*this, evt_streamtitle, st); }
//            break;
    }*/
    ...
    //  ÛÚËÏ ˆËÍÎ, ÔÓÍ‡ ÂÒÚ¸ ‰‡ÌÌ˚Â Ë ÔÓÍ‡ VS1053 „ÓÚÓ‚‡ Ëı ÔËÌËÏ‡Ú¸ (DREQ == HIGH)
    while (len && digitalRead(dreq_pin) == HIGH) {
        
        chunk_length = len;
        if (len > vs1053_chunk_size) {
            chunk_length = vs1053_chunk_size; // 32 ·‡ÈÚ‡
        }
    ...
    // ¬ÓÁ‚‡˘‡ÂÏ “ŒÀ‹ Œ ÚÓ, ˜ÚÓ Â‡Î¸ÌÓ ÛÒÔÂÎË Á‡ÚÓÎÍÌÛÚ¸ ‚ ˜ËÔ.
    // ≈ÒÎË DREQ ÛÔ‡Î ‚ LOW, Ï˚ ÔÓÒÚÓ ‚˚ıÓ‰ËÏ. ÕËÍ‡ÍËı vTaskDelay Á‰ÂÒ¸!
    return bytesDecoded;
}
//##################################################################
...
//            m_cat.nominalBitRate = (m_audioDataSize / m_decoder->getAudioFileDuration()) * 8;
        if (m_codec == CODEC_FLAC && FlacDecoder->getAudioFileDuration()) { // BITSTREAMINFO FLAC/OGG
            m_audioFileDuration = FlacDecoder->getAudioFileDuration();
            m_cat.nominalBitRate = (m_audioDataSize / FlacDecoder->getAudioFileDuration()) * 8;
        }
...
uint32_t Audio::getAudioCurrentTime(){
   ...
   m_audioCurrentTime = 8 * ((float)getAudioFilePosition() / (m_avr_bitrate));
...
//###################################################################
//uint16_t Audio::getVUlevel(uint16_t dimension){
uint16_t Audio::getVUlevel(){
//  if(!VS_PATCH_ENABLE) return 0;
  if(!_vuInitalized || !config.store.vumeter || !m_f_running) return 0;
  computeVUlevel();
  uint8_t L = map(vuLeft, config.vuThreshold, 0, 0, 255);
  uint8_t R = map(vuRight, config.vuThreshold, 0, 0, 255);
//  return (L << 8) | R;
  return (L << 8) + R;
...
//    if (resumeFilePos < (int32_t)m_audioDataStart) resumeFilePos = m_audioDataStart;
    ...
    // œÓ‚ÂˇÂÏ: ÂÒÎË ˝ÚÓ ÒÚ‡Ú ÚÂÍ‡ (ÌÂÚ ˇ‚ÌÓ„Ó Á‡ÔÓÒ‡ Ì‡ ÔÂÂÏÓÚÍÛ ‚ ÒÂÂ‰ËÌÛ),
    // ÚÓ ÔËÌÛ‰ËÚÂÎ¸ÌÓ Ò·‡Ò˚‚‡ÂÏ ÔÓÁËˆË˛ ‚ 0, ˜ÚÓ·˚ ÔÓ˜ËÚ‡Ú¸ Ù‡ÈÎ Ò Ò‡ÏÓ„Ó Ì‡˜‡Î‡!
    bool isStartOfTrack = (resumeFilePos <= (int32_t)m_audioDataStart);
    if (isStartOfTrack) {
//        resumeFilePos = m_audioDataStart;
    ...
    // ------- prepare InBuff ------
    m_f_allDataReceived = false;
    audioFileSeek(resumeFilePos); // “ÂÔÂ¸ ÚÛÚ ·Û‰ÂÚ ÒÚÓ„Ó 0 ÔË ÒÚ‡ÚÂ!
    InBuff.reset();
    ...
    // ≈ÒÎË ˝ÚÓ ÒÚ‡Ú Ù‡ÈÎ‡, Ì‡Ï Õ≈ ÌÛÊÌÓ ËÒÍ‡Ú¸ ÒËÌıÓÒÎÓ‚‡ Ë ÂÁ‡Ú¸ ·ÛÙÂ
    if (isStartOfTrack) {
        offset = 0; // ÕË˜Â„Ó ÌÂ ÓÚÂÁ‡ÂÏ, ÓÒÚ‡‚ÎˇÂÏ ‚ÂÒ¸ Ù‡ÈÎ Í‡Í ÂÒÚ¸
    }
    else {
        // ÀÓ„ËÍ‡ ‚˚‡‚ÌË‚‡ÌËˇ ÙÂÈÏÓ‚ ‡·ÓÚ‡ÂÚ “ŒÀ‹ Œ ÔË Â‡Î¸ÌÓÈ ÔÂÂÏÓÚÍÂ
        switch (m_codec) {
            case CODEC_M4A:   offset = m4a_correctResumeFilePos();  break;
            case CODEC_WAV:   offset = wav_correctResumeFilePos();  break;
            case CODEC_MP3:   offset = mp3_correctResumeFilePos();  break;
            case CODEC_FLAC:
                offset = flac_correctResumeFilePos();

/* 
            // ≈ÒÎË ˝ÚÓ ÒÚ‡Ú ÚÂÍ‡ Ò Ò‡ÏÓ„Ó Ì‡˜‡Î‡
            if (resumeFilePos <= (int32_t)m_audioDataStart) {
                // —ÂÈ˜‡Ò ‚ InBuff ‰‡ÌÌ˚Â ÎÂÊ‡Ú Ò 0-„Ó ·‡ÈÚ‡ (‚ÂÒ¸ ÏÛÒÓ + ‡Û‰ËÓ).
                //  —ÚÓÍ‡ `InBuff.bytesWasRead(offset)` ÌËÊÂ Ò‰‚ËÌÂÚ ÛÍ‡Á‡ÚÂÎ¸ ˜ÚÂÌËˇ 
                //  ‚ÔÂÂ‰ Ì‡ 8288 ·‡ÈÚ, ÓÒÚ‡‚Ë‚ ‚ ·ÛÙÂÂ ÚÓÎ¸ÍÓ ˜ËÒÚ˚È FLAC-ÔÓÚÓÍ.
                  
                //  Ã˚ ÏÓÊÂÏ ‡ÍÍÛ‡ÚÌÓ Ò‰ÂÎ‡Ú¸ ¯‡„ Ì‡Á‡‰ (´ÓÚÏÓÚ‡Ú¸ª 4 ·‡ÈÚ‡) 
                //  Ë Á‡ÔËÒ‡Ú¸ ÚÛ‰‡ "fLaC", ÔÂÂÔËÒ‡‚ ÔÓÒÎÂ‰ÌËÂ 4 ·‡ÈÚ‡ ÏÛÒÓ‡!
                
                if (offset >= 4) {
                    uint8_t* writePtr = InBuff.getReadPtr() + offset - 4;
                    writePtr[0] = 0x66; // 'f'
                    writePtr[1] = 0x4C; // 'L'
                    writePtr[2] = 0x61; // 'a'
                    writePtr[3] = 0xC3; // 'C'
                    ...
                    // ”ÏÂÌ¸¯‡ÂÏ offset Ì‡ 4. 
                    // “ÂÔÂ¸ bytesWasRead(offset) ÓÚÒÚÛÔËÚ Ì‡ 4 ·‡ÈÚ‡ ÏÂÌ¸¯Â,
                    // Ë ÓÚÔ‡‚Í‡ ‚ VS1053 Ì‡˜Ì∏ÚÒˇ ÚÓ˜ÌÂıÓÌ¸ÍÓ Ò Ì‡¯Ëı ·‡ÈÚ "fLaC"!
                    offset -= 4; 
                }
            }
*/
                break;
            case CODEC_VORBIS: offset = ogg_correctResumeFilePos(); break;
            case CODEC_OPUS:   offset = ogg_correctResumeFilePos(); break;
            default: return fail();
        }
    ...
    newFilePos += offset;
    m_decoder = false; // bool -> clear

    // --- leaf critical area ----------------------------------------------
    InBuff.bytesWasRead(offset); // œË ÒÚ‡ÚÂ ÚÛÚ 0, ·ÛÙÂ ÌÂ ÚÓ„‡ÂÏ!
    m_f_lockInBuffer = false;
    xSemaphoreGive(mutex_audioTaskIsDecoding);
...
int32_t Audio::mp3_correctResumeFilePos() {
    ...
    // —ÓÁ‰‡ÂÏ ÎÓÍ‡Î¸Ì˚È Ó·˙ÂÍÚ ‰ÂÍÓ‰Â‡, ÔÂÂ‰‡‚‡ˇ ÂÏÛ ÒÒ˚ÎÍÛ Ì‡ ÚÂÍÛ˘ËÈ ÍÎ‡ÒÒ Audio
    MP3Decoder tempDecoder(*this);

    while (true) {
//        steps = m_decoder->findSyncWord(pos, av);			// ?
        // ¬˚Á˚‚‡ÂÏ ÏÂÚÓ‰ ˜ÂÂÁ ÚÓ˜ÍÛ Û ÒÓÁ‰‡ÌÌÓ„Ó ÎÓÍ‡Î¸ÌÓ„Ó Ó·˙ÂÍÚ‡
        steps = tempDecoder.findSyncWord(pos, av);
        if (steps == 0) break;
        if (steps == -1) return -1;
        pos += steps;
        sumSteps += steps;
...
//        if (m_f_I2S_init) performAudioTask();
        if (m_decoder) performAudioTask();
// »ÒÔÓÎ¸ÁÛÂÏ ‚ÒÚÓÂÌÌ˚È Ï‡ÍÓÒ, „‡‡ÌÚËÛ˛˘ËÈ Á‡‰ÂÊÍÛ ‚ 1 ÚËÍ
...
//    gain_ramp();
    ...
    // ƒÎˇ FLAC (Ë ‰Îˇ MP3 ÚÓÊÂ ·Û‰ÂÚ ÔÓÎÂÁÌÓ) ÍÛÚËÏ ˆËÍÎ ÓÚÔ‡‚ÍË,
    // ÔÓÍ‡ ‚ ·ÛÙÂÂ InBuff ÂÒÚ¸ ‰‡ÌÌ˚Â Ë VS1053 „ÓÚÓ‚‡ Ëı ÔËÌËÏ‡Ú¸ (DREQ == 1)
    // Œ„‡ÌË˜ËÏ ˆËÍÎ ‡ÁÛÏÌ˚Ï ÔÂ‰ÂÎÓÏ (Ì‡ÔËÏÂ, 10 ËÚÂ‡ˆËÈ), ˜ÚÓ·˚ ÌÂ ‚˚Á‚‡Ú¸ ÒÚÓÓÊÂ‚ÓÈ Ú‡ÈÏÂ (Watchdog)
//    uint8_t safety_counter = 0;
//    while (InBuff.readSpace() > 0 && digitalRead(dreq_pin) && safety_counter < 10) {
//        playAudioData();
//        safety_counter++;
//    }
}
...
/*void Audio::performAudioTask() {
    if (m_decoder) {
        ...
        // ¿Õ¿ÀŒ√ “Œ–ÃŒ«¿ WOLLE: 
        // ≈ÒÎË ‡ÔÔ‡‡ÚÌ˚È ·ÛÙÂ VS1053 Á‡ÔÓÎÌÂÌ (DREQ == LOW), 
        // Ú‡ÒÍ ÔÓÒÚÓ ÒÔËÚ ÔÓ 20 ÏÒ, ÚÓ˜ÌÓ Ú‡Í ÊÂ, Í‡Í Û Wolle ‰Îˇ I2S.
        // ›ÚÓ ‰‡ÂÚ Ë‰Â‡Î¸Ì˚È Ò·ÓÒ WDT Ë ÓÒ‚Ó·ÓÊ‰‡ÂÚ ÔÓˆÂÒÒÓ.
        while (digitalRead(dreq_pin) == LOW) {
            vTaskDelay(20 / portTICK_PERIOD_MS);
            // «‰ÂÒ¸ Ì‡Ï ÌÂ ÌÛÊÂÌ playChunk(), Ú‡Í Í‡Í VS1053 Ë„‡ÂÚ Ò‡Ï‡!
        } 
        
        //  ‡Í ÚÓÎ¸ÍÓ DREQ ÒÚ‡Î HIGH (ÓÒ‚Ó·Ó‰ËÎÓÒ¸ ÏÂÒÚÓ ÏËÌËÏÛÏ Ì‡ 32 ·‡ÈÚ‡) ó
        // ‚˚Á˚‚‡ÂÏ playAudioData(), ˜ÚÓ·˚ ÔÓ‰ÍËÌÛÚ¸ ÌÓ‚Û˛ ÔÓˆË˛ ‰‡ÌÌ˚ı.
        playAudioData();
        
        xSemaphoreGive(mutex_audioTask);
        // gain_ramp(); // ÂÒÎË ËÒÔÓÎ¸ÁÛÂ¯¸ ÔÎ‡‚ÌÓÒÚ¸ „ÓÏÍÓÒÚË
        return;
        
    } else {
        // ÀÓ„ËÍ‡ ÔÓÒÚÓˇ (VU-ÏÂÚ)
        computeVUlevel(); 
        // gain_ramp();
        vTaskDelay(20);
        return;
