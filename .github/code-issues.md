...
- [ ] 16. Plugin System — Dead Infrastructure, Remove Entirely
...
### 1.1 Background: How SPI Devices Are Currently Configured

The codebase supports four SPI-connected hardware devices: the **display**, the **VS1053 audio decoder**, the **SD card**, and the **XPT2046 touchscreen**. Each device needs an SPI bus. Today, each device gets one of two choices — the "default" SPI bus or the "HSPI" bus — via a single boolean macro:

| Device | Boolean flag | Default (false) | Alternative (true) |
|---|---|---|---|
| Display | `DSP_HSPI` | default SPI (`&SPI`) | shared `SPI2(HSPI)` |
| VS1053 | `VS_HSPI` | default SPI (`&SPI`) | shared `SPI2(HSPI)` |
| SD card | `SD_HSPI` | default SPI | `SDSPI(HSPI)` |
| Touchscreen | `TS_HSPI` | default SPI | `TSSPI(HSPI)` |

The hardware-level SPI pin assignments for each bus come entirely from the board definition file (e.g., the board variant's `pins_arduino.h`). **You cannot override individual SPI pins per-device in `myoptions.h`** — for display and VS1053 there is no escape hatch at all. SD card and touchscreen have partial escapes (`SD_SPIPINS` and `TS_SPIPINS`), but these are tuple macros rather than individual pin defines and still depend on the `HSPI` bus constant from the board definition.

---

### 1.2 The VSPI / FSPI Naming Trap

On the **original ESP32**:
- `VSPI` = SPI bus 3, default pins: MOSI=23, MISO=19, SCK=18
- `HSPI` = SPI bus 2, default pins: MOSI=13, MISO=12, SCK=14

On **ESP32-S3**:
- `VSPI` does not exist in the Arduino ESP32 library
- `FSPI` nominally refers to SPI0/1 which is reserved for flash in the ESP-IDF driver model
- The Arduino `SPI` object on S3 actually maps to **SPI2** (`SPI2_HOST`) using the chip's default SPI2 pins: MOSI=11, MISO=13, SCK=12
- `HSPI` on S3 maps to SPI3 with entirely different pin assignments

The codebase papers over this mismatch with one line in `options.h`:
```cpp
#if !defined(CONFIG_IDF_TARGET_ESP32)
  #define VSPI FSPI
#endif
```
This shim makes `VSPI` expand to `FSPI` on S3, which in practice works because `SPI.begin()` on S3 initializes SPI2 (MOSI=11, MISO=13, SCK=12). However the `options.h` fallback comments still say:
```
// use HSPI for ... (miso=12, mosi=13, clk=14)
```
Those are **ESP32 HSPI pin numbers** — completely wrong for S3. Any user reading that comment and wiring to pins 12/13/14 on an S3 gets non-functional hardware. The shim compiles but does nothing to correct this documentation problem.

---

### 1.3 The `I2S_DOUT==255` Decoder Selection Gate

The codebase uses `I2S_DOUT==255` (a disabled/illegal pin value) as a proxy for "VS1053 is the active decoder." Setting `I2S_DOUT 255` disables I2S, which the code reads as "therefore VS1053 is active."

Examples from the codebase:
```cpp
#if I2S_DOUT!=255 || I2S_INTERNAL   // player.h: include I2S library
#if I2S_DOUT==255                   // audioVS1053Ex.cpp: compile VS1053 code
#if I2S_DOUT==255                   // display.cpp: VS1053 VU meter gate
```

This is **exclusion-based decoder selection** — the reader must know that pin 255 means "disabled" and that I2S being disabled implies VS1053. It is inverted logic. A misconfig where someone sets `I2S_DOUT 255` for a legitimate non-VS1053 reason (e.g., no I2S output pin, USB audio only) would incorrectly trigger VS1053 code paths. The only guard preventing simultaneous activation is an `#error` in `options.h`:
```cpp
#if (I2S_DOUT!=255) && (VS1053_CS!=255)
  #error both I2S_DOUT and VS1053_CS are active
```
A positive decoder identity macro would be clearer and safer than this double-negative.

Additionally, it makes VS1053 and I2S a **binary exclusive choice**. Future hardware that might need to use both (e.g., VS1053 for FM radio decode + I2S DAC for output) cannot be expressed in the current design.

---

### 1.4 Missing Per-Pin Overrides for Display and VS1053 SPI

SD card has `SD_SPIPINS`; touchscreen has `TS_SPIPINS`. **Display and VS1053 have no equivalent.**

If you are building on an ESP32-S3 dev board where the SPI display is wired to non-default pins, there is no way to remap the display SPI in `myoptions.h`. Same for VS1053. Real-world impact: several ESP32-S3 dev boards with built-in displays use SPI wiring that doesn't match S3 SPI2 defaults (MOSI=11, MISO=13, SCK=12). If that board has no Arduino-ESP32 variant definition, or if the board definition doesn't match the display wiring, the user is stuck.

The comment in `myoptions.h` even acknowledges this for the VS1053 board:
```cpp
/* When using SPI Displays, trying to use same SPI MOSI, SCK, MISO as VS1053 doesn't work */
```
This is a workaround acknowledgment of the conflict, not a fix. The right solution is to let each device specify its own SPI pins.

---

### 1.5 The Shared `SPI2(HSPI)` Global

When any of `DSP_HSPI`, `TS_HSPI`, or `VS_HSPI` is `true`, `main.cpp` creates:
```cpp
SPIClass SPI2(HSPI);
```
This **single global** is shared by every HSPI device simultaneously. Consequences:
- Display and VS1053 cannot independently use custom SPI pins — they share the one HSPI bus instance
- No per-device pin override is possible via this global
- `SPI2` is a confusing name coincidentally matching the ESP32's SPI2 bus name (which is unrelated)
- The `extern SPIClass SPI2` declaration in `config.h` spreads this dependency into the core module

---

### 1.6 `SD_SPIPINS` / `TS_SPIPINS` Tuple Antipattern

```cpp
#define SD_SPIPINS  21, 13, 14   // SCK, MISO, MOSI (in myoptions.h)
// ...
SDSPI.begin(SD_SPIPINS);         // expands to: SDSPI.begin(21, 13, 14)
```

This is a macro token-list trick that injects comma-separated values directly into a function call. Problems:
- The three values have no names at the call site — you must know that `begin(SCK, MISO, MOSI)` is the argument order
- It is inconsistent with every other pin macro in `options.h`, which are individual `#define`s with descriptive names
- It cannot be validated at compile time (range checks, pin-conflict checks)
- Extending the pattern to add a fourth parameter requires changing syntax at every call site

Recommendation: replace `SD_SPIPINS` with three individual macros `SD_SCK`, `SD_MISO`, `SD_MOSI`; replace `TS_SPIPINS` with `TS_SCK`, `TS_MISO`, `TS_MOSI`. This aligns with the `I2S_BCLK`/`I2S_LRC`/`I2S_DOUT` pattern already used for I2S pins.

---

### 1.7 Recommended Migration — Four Phases

**Phase 1 — `myoptions.h`: Require explicit per-pin definitions**

`options.h` should **not** silently supply default pin values for SPI devices — pins belong in `myoptions.h`. The role of `options.h` is to derive gating macros from what `myoptions.h` declared and to check for conflicts. For each SPI device, `options.h` gains a commented reference block (no `#ifndef` fallback values) followed by a derivation:

```c
// VS1053 SPI pins — must be defined in myoptions.h for VS1053 to be active.
// No defaults are provided: the correct pins depend entirely on your wiring.
// ESP32 VSPI recommended:   MOSI=23, MISO=19, SCLK=18
// ESP32-S3 SPI2 recommended: MOSI=11, MISO=13, SCLK=12
// NOTE: if using a SPI TFT display, it is strongly recommended that the VS1053
//       be wired to a DIFFERENT SPI bus to avoid contention. See §1.10.
```

Repeat the same documentation pattern for `DSP_MOSI`/`DSP_MISO`/`DSP_SCLK` (display SPI), `SD_SCK`/`SD_MISO`/`SD_MOSI`, and `TS_SCK`/`TS_MISO`/`TS_MOSI`. Remove the old `*_HSPI` boolean fallback defaults and the `#define VSPI FSPI` shim (see §1.2).

**Phase 2 — `options.h`: Derive positive decoder identity defines**

At the end of the pin-definition section, `options.h` derives two gating macros based solely on whether the required pins are fully defined:
...
#if defined(VS1053_CS) && (VS1053_CS != 255) && \
    defined(VS1053_DCS) && defined(VS1053_DREQ) && \
    defined(VS1053_MOSI) && defined(VS1053_MISO) && defined(VS1053_SCLK)
  #define USE_AUDIO_VS1053
#endif

#if defined(I2S_DOUT) && (I2S_DOUT != 255) && defined(I2S_BCLK) && defined(I2S_LRC)
...
The `#undef` guards at the top prevent stale values if `options.h` is somehow included twice, or if a user mistakenly defines these directly. Once these derived macros exist, replace all `#if I2S_DOUT==255` in the codebase with `#if defined(USE_AUDIO_VS1053)` and `#if I2S_DOUT!=255` with `#if defined(USE_AUDIO_I2S)`. The old `#error` in `options.h` is subsumed by the new check above.

Note: `I2S_DOUT` currently has a default of 27 in `options.h` — this is a legacy ESP32 pin. For the new gate to work correctly, the `I2S_DOUT` default should be changed to `255` (disabled), consistent with the `VS1053_CS 255` convention. All existing `myoptions.h` profiles already define `I2S_DOUT` explicitly so no board profile breaks.

**Phase 3 — Per-module SPI initialization using the new macros**

- **VS1053** (`player.cpp`): Replace `#if VS_HSPI / &SPI2 / &SPI` constructor selection with a module-local `SPIClass vsSPI(SPI2_HOST)` (or equivalent bus constant) initialized in `Player::init()` via `vsSPI.begin(VS1053_SCLK, VS1053_MISO, VS1053_MOSI, -1)`, then pass `&vsSPI` to the `Audio` constructor. After Issue 2 Step E2, the same pin macros feed the upstream 7-param constructor directly.
- **Display** (each `displayXXX.cpp`): Replace the `#if DSP_HSPI / &SPI2 / &SPI` branch with an unconditional constructor that passes a local `SPIClass` instance configured with `DSP_MOSI`/`DSP_MISO`/`DSP_SCLK` — or use the SPIClass pointer constructor where the display library supports it.
- **SD card** (`sdmanager.cpp` / `config.cpp`): Remove the `#if defined(SD_SPIPINS) || SD_HSPI` branch; always call `SDSPI.begin(SD_SCK, SD_MISO, SD_MOSI)`.
- **Touchscreen** (`touchscreen.cpp`): Same as SD — remove `#ifdef TS_SPIPINS`; always call `TSSPI.begin(TS_SCK, TS_MISO, TS_MOSI)`.

**Phase 4 — Cleanup**

- Remove `DSP_HSPI`, `VS_HSPI`, `SD_HSPI`, `TS_HSPI` from `options.h`
- Remove `#if DSP_HSPI || TS_HSPI || VS_HSPI` / `SPIClass SPI2(HSPI)` from `main.cpp`
- Remove `extern SPIClass SPI2` from `config.h`
- Update all `myoptions.h` board profiles to use new per-pin macros (Rule #3 — explicit confirmation required)
- Update `builds/*/myoptions.h` contributor profiles

---

### 1.8 Impact on Issue 30 (Audio Library Migration)

This is why §29 is a prerequisite for §30 Step E2. The upstream `VS1053` constructor (nsteplanets `ESP32-vs1053_ext-master`) requires 7 explicit parameters:
```cpp
VS1053(cs, dcs, dreq, spi_bus_num, mosi, miso, sclk)
```
Once `VS1053_MOSI`, `VS1053_MISO`, `VS1053_SCLK` are defined by `myoptions.h` (and validated by `options.h` into `USE_AUDIO_VS1053`), the `AudioEncoderShim` constructor (§2.4) can fill all seven positions. The bus number comes from a local `SPIClass` instance (Phase 3 above) whose constructor was passed the numeric bus constant.

Without §1 Phase 1 and 2 completing first, Step E2 of Issue 2 cannot compile — `VS1053_MOSI`, `VS1053_MISO`, and `VS1053_SCLK` simply do not exist as symbols.

---

### 1.9 SPI Bus Sharing, Contention, and Speed Override Macros

#### Can two devices share one SPI bus?

Technically yes. Arduino's `SPIClass` uses `beginTransaction(SPISettings(clock, ...))` / `endTransaction()` to lock the bus, apply per-device settings, and release. Each device can have a different clock speed and they take turns. This is the standard Arduino SPI sharing contract — the VS1053 library, Adafruit TFT drivers, and the SD library all implement it correctly.

In practice though, **an audio streaming chip sharing a bus with a display causes real problems**:

- The VS1053 is serviced by a FreeRTOS audio task that runs continuously, calling `beginTransaction` / `endTransaction` in a tight loop to push compressed audio data (8 MHz, non-stop). On a shared bus the display task has to queue behind every audio transaction. At 8 MHz with ~32-byte audio packets, the VS1053 occupies the bus for tens of microseconds out of every hundred. Display redraws — which need many sequential 4-byte writes at 24–40 MHz — become fragmented and slow. The result is display lag or corruption.
- During VS1053 initialization (`Audio::begin()`), the library drives the bus at **0.25 MHz** for hundreds of milliseconds, performing multiple slow register write sequences. If this overlaps with display init, the display may receive garbage.
- Some display libraries call `SPI.begin()` inside their constructor or `initDisplay()`. If another module has already called `spi->begin()` on the same bus, a second `begin()` may silently reconfigure the bus pins — especially if the two callers use different `SPIClass` instances mapped to the same underlying hardware SPI peripheral.

**Why SD and touchscreen were moved off the default bus**: SD card read bursts (20–40 MHz, variable length) and XPT2046 ADC reads (2.5 MHz) add to bus contention. Both devices running on a separate bus (HSPI) eliminated the contention with display updates. This is the correct approach and Phase 3 of §1.7 preserves it — each device gets its own `SPIClass` instance initialized with its own pin set.

#### Speed override macros — what exists today

| Device | Speed macro | Default | Override location | Notes |
|---|---|---|---|---|
| Display | `DEF_SPI_FREQ` | varies by driver (8–40 MHz) | `options.h` (commented) or `myoptions.h` | Some display drivers (e.g., `displayILI9341.cpp`) use the library's own `SPI_DEFAULT_FREQ` (40 MHz) and ignore `DEF_SPI_FREQ` entirely |
| SD card | `SDSPISPEED` | 20 MHz (ESP32) / 40 MHz (S3) | `options.h` default or `myoptions.h` | Well-documented, user-overridable |
| VS1053 control | (none) | 0.25 MHz (init) → up to 6.7 MHz (post-init) | **Not user-overridable** | Hardcoded `SPISettings` in `audioVS1053Ex.cpp` lines ~337, ~361 |
| VS1053 data | (none) | 8 MHz | **Not user-overridable** | Hardcoded `SPISettings` in `audioVS1053Ex.cpp` line ~338 |
| XPT2046 touch | (none) | 2.5 MHz | Not user-overridable | Managed internally by the XPT2046 library |

`DEF_SPI_FREQ` is the one tuning knob users have for display SPI speed. It is documented (commented-out) in `options.h` line ~118. If a board's display is glitchy or blank, lowering `DEF_SPI_FREQ` is the first thing to try. The VS1053 data speed is not overridable without patching the library source — an argument for the upstream migration (Issue 2) where the shim could potentially expose a speed configuration path.

#### Compile-time conflict detection: what we can and cannot check

Pin collision between devices (same MOSI/MISO/SCLK but different CS pins = shared bus, fine; same MOSI and different SCK = misconfiguration) is theoretically detectable with a large `#if` forest, but the combinatorial space is large and the maintenance cost high. More importantly, some overlaps are intentional (two devices on the same bus by design), so blocking all pin overlaps at compile time would reject valid configurations.

**What `options.h` can reasonably check:**
- CS pins: if two devices share the same CS, both would be simultaneously selected and fight — this IS always a misconfiguration. Worth a targeted `#error`:
  ```c
  #if defined(USE_AUDIO_VS1053) && (SD_CS != 255) && (VS1053_CS == SDC_CS)
    #error VS1053_CS and SDC_CS are the same pin — devices cannot share a chip-select.
  #endif
  ```
- Similarly for `VS1053_CS == TFT_CS`, `SDC_CS == TFT_CS`, etc.

**What `options.h` should not try to check:**
- Whether MOSI/MISO/SCLK pins overlap between devices — this is allowed (shared bus) and checking it would flag intentional configurations as errors
- Runtime contention — cannot be detected at compile time

**Documentation approach (recommended):** Add a clear warning block as a comment in `options.h` in the VS1053 and display SPI pin sections — similar to the note already in `myoptions.h`:
```
// NOTE: When using a SPI TFT display AND VS1053, using the same SPI bus for both
// causes display lag and possible corruption due to continuous VS1053 audio streaming.
// It is strongly recommended to wire each device to a different hardware SPI bus.
```

---

### 1.10 Implementation Scope and Rules

Files touched: `src/core/options.h`, `src/main.cpp`, `src/core/config.h`, `src/core/player.cpp`, `src/core/sdmanager.cpp`, `src/core/touchscreen.cpp`, all `src/displays/displayXXX.cpp` drivers (~12 files), `myoptions.h`, all `builds/*/myoptions.h`.

- **Rule #1**: Each phase is >50 lines and multi-file. Each phase requires Plan mode confirmation before implementation begins.
- **Rule #3**: `options.h` and `myoptions.h` require explicit user confirmation before any edit.
- Phases 1 and 2 can be done independently and don't require hardware testing. Phase 3 requires hardware testing of each environment after its module is updated. Phase 4 is cleanup only after Phase 3 is fully validated.
...
## [ ] 16. Plugin System — Dead Infrastructure, Remove Entirely `[LOW]`
...
### 16.1 The `.ino` files — Dead Arduino IDE Remnants `[TRIVIAL]`
...
### 16.2 `backlightControls` example — Superseded by §15 `[LOW]`
...
### 16.3 `deepSleep` example — Feature Gap, Known S3 Bug `[LOW]`
...
### 16.4 `helloWorld` example — Developer Reference Only `[TRIVIAL]`
...
### 16.5 Plugin Infrastructure Removal — Full Scope `[LOW]`
