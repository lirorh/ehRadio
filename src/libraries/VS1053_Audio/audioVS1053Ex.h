...
// [Maleksm graft] task coordination
SemaphoreHandle_t   mutex_playAudioData = NULL;
SemaphoreHandle_t   mutex_audioTask     = NULL;
TaskHandle_t        m_audioTaskHandle = nullptr;
bool                m_f_audioTaskIsDecoding = false;  // true while task's playAudioData() is decoding
bool                m_f_lockInBuffer = false;          // true while loop() is manipulating InBuffer
bool                m_f_firstPlayCall = true;          // one-time init in playAudioData()
    ...
    // [Maleksm graft] variables needed by task-coordinated playAudioData()
    bool            m_f_stream = false;             // stream ready for output?
    bool            m_f_eof = false;                // end of file reached
    bool            m_f_ID3v1TagFound = false;      // ID3v1 tag found
    uint32_t        m_sumBytesDecoded = 0;          // used for streaming position tracking
    uint32_t        m_bytesNotDecoded = 0;          // pictures or other non-audio data in stream
    int16_t         m_validSamples = {0};           // samples still to play (I2S leftover, may be 0 for VS1053)
...
protected:
    ...
    // [Maleksm graft] task infrastructure — replaces the no-op stub
...
private:
    // [Maleksm graft] FreeRTOS task for anti-skip audio feeding
